class Robot {
  ...
  this.zwart = zwart;                           // Variabelen specifiek voor de Robot
...
update(){
  this.robotX = mouseX-65;                      // Positie Muis blijven updaten
...
show(){
   push();
  translate(this.robotX,this.robotY);           // Robot verplaatsen naar positie Muis
  let ms = millis();                            // Timer starten

//KLEUR OMLIJNING ZWART
  stroke(this.zwart);                           // Standaard omlijningskeur instellen


  if (ms % 100 > 50) {            // Zoveel mogelijk delen door 100 zolang er een geheel getal over blijft
...
push();
  //Schouder Rechts
    ...
    this.angle = constrain(this.angle, 1/10, PI / 2);   // constraint de angle in een range van 1/10 tot 90°
    this.angle2 = constrain(this.angle2, 0, PI/4);      // constraint de angle2 in een range van 0 tot 45°
...
push();
    ...
    for(let i=0; i<65; i+=5){                                 // Rupsbanden met schuine zijden! (y-coördinaten zijn niet zelfde)
...
push();
   ...
   strokeWeight(2);
    ...
    line(248,291+i-this.posRups,278,287+i-this.posRups);      // Rupsbanden met schuine zijden! (y-coördinaten zijn niet zelfde)
    }
pop();
